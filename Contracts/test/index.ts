...
Router Address: 0x932102Bc1916f9d74E271dB6685aba0276f112F2 
  */
  ...
  before('Initialize and Deploy SmartContracts', async () => {
      ...
      const CoinVestingVaultArtifact = await ethers.getContractFactory("PurchasedCoinVestingVault");
      ...
      console.log('ZSalesToken Deployed at  ', zsalesToken.address );
      ...
      let zsalesNftToken = await ZSalesTokenArtifact.deploy();
      await zsalesNftToken.deployed();
      console.log('zsalesNftToken Deployed at  ', zsalesNftToken.address );
...
Using Campaign List Deployed at   0xA1C16951Ad300Be8D2fb2976303d52ABa5Aa1b25*/
  ...
  async function createNewCampaign( start: any,end: any, purchaseTokenAddress="0x0000000000000000000000000000000000000000"){
    const [owner] = await ethers.getSigners();
  ...
  it("Should create new campaign", async function () {
    ...
    const createCampaignResult = await createNewCampaign(thirtySecondsTime, thirtyDaysLater,'0x0000000000000000000000000000000000000000');
  ...
  it('stops duplicates contract for the same token except when cancelled', async() => {
      ...
      try{
        const createCampaignTx = await campaignFactory.createNewCampaign(token.address,
        usdcToken.address, //"0x0000000000000000000000000000000000000000",
  ...
  it('submits initial token to contract successfully', async() => {
    ...
    try{
      ...
      let totalTokenTobeTransferred = ethers.utils.parseEther( 1.2 * parseFloat(ethers.utils.formatUnits(vestingTokens)) +'') //2%            
  ...
  it('accepts Native Coin bids succesfully for non whitelisted sale', async() => {
    ...
    try{
      ...
      console.log('Min alloc: ', await cmp.purchaseTokenAddress())
      console.log('Min alloc: ', await cmp.minAllocationPerUser())
      console.log('Bid: ', ethers.utils.parseEther("0.1"))
      console.log('Bid 2: ', ethers.utils.formatUnits( ethers.utils.parseEther("0.1"), 'wei'))

      let campaignContractAsNewSigner = cmp.connect(newSigner);

      let tx = await campaignContractAsNewSigner.submitBid([],0, {
        value: ethers.utils.formatUnits( ethers.utils.parseEther("0.1"), 'wei')
      } );
  ...
  it('accepts Erc20 Coin bids succesfully for non whitelisted sale', async() => {
    ...
    try{
      ...
      let campaignStatus = await cmp.getCampaignStatus();      
      let vestingTokens = await cmp.totalTokensExpectedToBeLocked();  
      let fees = 1.2 * parseFloat(ethers.utils.formatUnits(vestingTokens));
      let totalTokenTobeTransferred = ethers.utils.parseEther( 1.2 * parseFloat(ethers.utils.formatUnits(vestingTokens)) +'') //2%
      
      // Approve contract with correct allowance = totalTokensExpectedToBeLocked + zsaleFees
      let txAllowance = await token.approve(campaignFactory.address, totalTokenTobeTransferred);
      await txAllowance.wait();
      ...
      expect(parseFloat( camapignAfterBalance)).to.gt(parseFloat(campaignStartBalance));
      expect(txRes.confirmations).to.gt(0);
